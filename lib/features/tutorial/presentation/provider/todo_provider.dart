...
class TodoNotifier extends _$TodoNotifier {
  ...
  Future<void> onLoadMore() async {
    ...
    if (currentState == null ||
        currentState.hasNextPage == false ||
    ...
    try {
      final todos = await ref
          ...
          .fetchTodos(page: currentState.page + 1, limit: limit);
    ...
    try {
      ...
      state = AsyncValue.data(
        state.value!.copyWith(
          data: state.value!.data.where((todo) => todo.id != id).toList(),
        ),
      );
    } catch (e) {
      print("Error: $e");
